...
def getAngle(x,y):
    if x==0 and y==0:
        ...
        return 180*(7/22)*math.atan(y/x)
    elif x>0 and y<0:
        return 270+180*(7/22)* math.atan(abs(y)/x)
    elif x<0 and y<0:
         return 180+180*(7/22)* math.atan(abs(y)/abs(x))
    elif x<0 and y>0:
         return 90+180*(7/22)* math.atan(y/abs(x)) 
...
mw=3
print(mh,mw)
xmask=np.zeros((mh, mw),'uint8')
ymask=np.zeros((mh, mw),'uint8')
...
oriImage=np.zeros((h,w),'uint8')
for i in range(h):
    for j in range(w):
        oriImage[i][j]=getAngle(grayImagex[i][j],grayImagey[i][j])

oriImage=selfIntensityNormalize(oriImage)

sumweight=0
for i in range(h):
    for j in range(w):
        sumweight+=oriImage[i][j]
